...
## Если приложение падает: `GLIBC_2.38 not found` (sqlite3)

Исправлено в репозитории: используется **sqlite3@5.1.7** и пересборка на Linux при `npm install`.

Сделайте **Redeploy** после последнего push. Сборка может занять 2–4 минуты (компиляция sqlite3).
